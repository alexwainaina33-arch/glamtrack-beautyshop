...
import { Plus, ChevronLeft, ChevronRight, Clock, User, Phone, X, Check, AlertCircle, Scissors } from 'lucide-react'
...
const TOTAL_HOURS = 13 // 8am to 9pm
...
export default function AppointmentsPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const [appts, svcs, stf, custs] = await Promise.all([
        ...
        pb.collection(C.SERVICES).getList(1, 200, { filter: `shop_id="${shop.id}" && is_active=true`, sort: 'name' }).then(r => r.items),
        pb.collection(C.STAFF).getList(1, 200, { filter: `shop_id="${shop.id}" && is_active=true`, sort: 'name' }).then(r => r.items),
        pb.collection(C.CUSTOMERS).getList(1, 500, { filter: `shop_id="${shop.id}"`, sort: 'name' }).then(r => r.items),
      ])
  ...
  const openEdit = (appt) => {
    ...
    setForm({
      ...
      notes: appt.notes || '', deposit_paid: appt.deposit_paid || 0, status: appt.status || 'scheduled'
    })
                  ...
                  return (
                    <div key={appt.id}
                      ...
                      style={{
                        ...
                        transition: 'all 0.15s', boxShadow: '0 1px 4px #0001',
                        zIndex: 1
                      }}
              ...
              return (
                <>
                  ...
                  <div style={{ padding: '20px 24px' }}>
                    ...
                    {appt.notes && (
                      <div style={{ marginTop: 12, background: '#fef5f7', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#6b4050', fontStyle: 'italic' }}>
                        📝 {appt.notes}
                      </div>
                    )}
